...
// ─── Cluster Mode for 1000+ concurrent users ───
...
if (cluster.isPrimary) {
  ...
  initDatabase().then((db) => {
    ...
    app.get('/dashboard', requireAuth, (req, res) => {
      ...
      // Get some stats
      const userCount = queryScalar(db, 'SELECT COUNT(*) FROM users');
      const logCount = queryScalar(db, 'SELECT COUNT(*) FROM terminal_logs');
    ...
    app.get('/terminal', requireAuth, (req, res) => {
      let logs = [];
      try {
        logs = queryAll(db, 'SELECT * FROM terminal_logs ORDER BY timestamp DESC LIMIT 50');
      } catch (e) { /* silent */ }
